...
def TrainModel_GT(hmm_parameters, weights, obs, chr_index, call_index, post_file, log_file, w,  not_est_starting_prob,
                  not_est_trans = False, maxiterations = 1000, epsilon = 0.0001, window_size = 1000,phased = False):  # w window index
    """
    ...
    for chr in range(n_chr):
        ...
        Z_ = np.zeros((n_phase,n_windows_, n_states))  # P(Z | O)
        print(Z_.shape)
        Z.append(Z_)
    previous_ll = -np.inf
    for iteration in range(maxiterations):
        ...
        top = np.zeros(2)
        bot = np.zeros(2)
        new_trans_ = np.zeros((n_chr, n_phase, n_states, n_states))
        new_ll = 0
        normalize = []
        for chr in range(n_chr):
            for phase in range(len(obs[chr])):
                len_weights = len(weights[chr])
                emissions = Emission_probs_poisson(hmm_parameters.emissions, obs[chr][phase], weights[chr])
                ...
                top[0] += np.sum(forward_probs[[call_index[chr]], 0] * backward_probs[[call_index[chr]], 0] * obs[chr][phase][call_index[chr]])
                bot[0] += np.sum(forward_probs[[call_index[chr]], 0] * backward_probs[[call_index[chr]], 0] * (weights[chr][call_index[chr]]) )
                
                top[1] += np.sum(forward_probs[[call_index[chr]], 1] * backward_probs[[call_index[chr]], 1] * obs[chr][phase][call_index[chr]])
                ...
                Z[chr][phase][:] = forward_probs * backward_probs 
                normalize.append(np.sum(Z[chr][phase], axis = 0))     # sum over posterior across chrs
                # Update Transition probs
                if not not_est_trans:
                    for state1 in range(n_states):
                        for state2 in range(n_states):
                            new_trans_[chr][phase][state1, state2] = np.sum( forward_probs[:-1,state1]  * hmm_parameters.transitions[state1, state2] * emissions[1:,state2] * backward_probs[1:,state2] / scales[1:] )
        new_emissions_matrix = top/bot
        if not not_est_trans:
            new_trans = np.sum(new_trans_, 0).sum(axis = 0)
