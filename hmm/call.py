...
"""
#sligthly different, easier for comparison
def Call_LS(posterior, out_file, window_size = 1000):
    post_all = pd.read_csv(posterior)
    chr_all = post_all.Chr.unique()
    segments = []
    if out_file is None:
        ssss = posterior_file+f".called"
    else:
        ssss = out_file
    with open(ssss, 'w') as f:
        print("chrom,start,end,score", file = f)
        for chr in chr_all:
            post_chr = post_all[post_all.Chr == chr].reset_index(drop=True)
            start = np.array(post_chr.Window)[0]
            length = len(post_chr)
            ...
            for (state, start_index, length_index) in find_runs(state_with_highest_prob):
                ...
                mean_prob = round(np.mean(post_chr_T[state, start_index:(start_index+length_index)]), 5)
                if state == 1:
                    print(chr, start_genome, end_genome, mean_prob, sep = ',', file=f) 
def get_runs(posterior_file, penalty=0.2):
    posterior_ = pd.read_csv(posterior_file)
    frags = []
    chr = list(set(posterior_.Chr.to_list()))
    phase = list(set(posterior_.phase.to_list()))
    for i in chr:
        for phase_ in phase:
            pp = posterior_[(posterior_['Chr'] == i)& (posterior_['phase'] == phase_)].reset_index(drop=True)
            ...
            while True:
                ...
                if pos_max != pos_min:
                    frags.append((i, phase_,id_[pos_min]+s, id_[pos_max]+s, p[pos_max] - p[pos_min]))
                p0[pos_min : (pos_max + 1)] = 0
    out_ =  pd.DataFrame(frags, columns=["chrom","phase","start", "end", "score"])
